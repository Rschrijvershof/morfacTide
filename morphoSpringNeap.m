function OUT = morphoSpringNeap(input,lat,varargin)
% Morphological tide toolbox (morphoTide)
%
% OUT = morphoSpringNeap(input,lat,varargin)
%
...
% HistogramScaling  Logical switch (1 or 0)
%               (e.g. 10 years)
...
addpath('C:\Users\schri050\OneDrive - Wageningen University & Research\MyDoc\02_software\Matlab\code\tools\tide\t_tide_v1.4beta');
...
elseif iscell(input)
    OUT.td = (OPT.tStart:datenum(0,0,0,0,10,0):OPT.tStart+365)';
end
OUT.th      = OUT.td*24;
...
%         [pha2,~,~,~,~] = anglemean([deg2rad(D.mor.O1.pha),deg2rad(D.mor.K1.pha)]);
%         pha2 = rad2deg(pha2);
        
        pha2 = rad2deg(mean_angle(deg2rad([SN.D1.phi(1);SN.D1.phi(2)])));
...
%         SN.C1.phi   = mod(atan2d(imag(pha),real(pha)),360);   % to check
        ...
        SN.mor.cycle = SN.mor.hC1D2modD4D6D8;
        % Rewrite the signal as a linear combination of sines and cosinse
        % to get rid of the phases. This
        SN.C1.a = SN.C1.A*cos(SN.C1.phi);
        SN.C1.b = SN.C1.A*sin(SN.C1.phi);
        SN.D2.a = (SN.D2.A(1) + SN.Dsn.Avar).*cos(SN.D2.phi(1));
        SN.D2.b = (SN.D2.A(1) + SN.Dsn.Avar).*sin(SN.D2.phi(1));
        SN.D4.a = SN.D4.A(1)*cos(SN.D4.phi(1));
        SN.D4.b = SN.D4.A(1)*sin(SN.D4.phi(1));
        SN.D6.a = SN.D6.A(1)*cos(SN.D6.phi(1));
        SN.D6.b = SN.D6.A(1)*sin(SN.D6.phi(1));
        SN.D8.a = SN.D8.A(1)*cos(SN.D8.phi(1));
        SN.D8.b = SN.D8.A(1)*sin(SN.D8.phi(1));
        
        SN.mor.cycle2 = ...
            SN.C1.a .* cos(SN.C1.omega.*SN.mor.ts) + SN.C1.b .* sin(SN.C1.omega.*SN.mor.ts) + ...
            SN.D2.a .* cos(SN.D2.omega(1).*SN.mor.ts) + SN.D2.b .* sin(SN.D2.omega(1).*SN.mor.ts) + ...
            SN.D4.a .* cos(SN.D4.omega(1).*SN.mor.ts) + SN.D4.b .* sin(SN.D4.omega(1).*SN.mor.ts) + ...
            SN.D6.a .* cos(SN.D6.omega(1).*SN.mor.ts) + SN.D6.b .* sin(SN.D6.omega(1).*SN.mor.ts) + ...
            SN.D8.a .* cos(SN.D8.omega(1).*SN.mor.ts) + SN.D8.b .* sin(SN.D8.omega(1).*SN.mor.ts);
        
        %% Scaling procedure
        if OPT.HistogramScaling
            ...
            % Scaling on the semi-diurnal amplitudes
            ...
            for k = 1:length(fac)
                ...
                for m = 1:length(fac)
                    ...
                    Z2 = ...
                        SN.C1.a .* cos(SN.C1.omega.*SN.mor.ts) + SN.C1.b .* sin(SN.C1.omega.*SN.mor.ts) + ...
                        fac(k) .* SN.D2.a .* cos(SN.D2.omega(1).*SN.mor.ts) + fac(m) .* SN.D2.b .* sin(SN.D2.omega(1).*SN.mor.ts) + ...
                        SN.D4.a .* cos(SN.D4.omega(1).*SN.mor.ts) + SN.D4.b .* sin(SN.D4.omega(1).*SN.mor.ts) + ...
                        SN.D6.a .* cos(SN.D6.omega(1).*SN.mor.ts) + SN.D6.b .* sin(SN.D6.omega(1).*SN.mor.ts) + ...
                        SN.D8.a .* cos(SN.D8.omega(1).*SN.mor.ts) + SN.D8.b .* sin(SN.D8.omega(1).*SN.mor.ts);
            ...
            SN.mor.hC1fD2modD4D6D8 = ...
                SN.C1.a .* cos(SN.C1.omega.*SN.mor.ts) + SN.C1.b .* sin(SN.C1.omega.*SN.mor.ts) + ...
                fD2a .* SN.D2.a .* cos(SN.D2.omega(1).*SN.mor.ts) + fD2b .* SN.D2.b .* sin(SN.D2.omega(1).*SN.mor.ts) + ...
                SN.D4.a .* cos(SN.D4.omega(1).*SN.mor.ts) + SN.D4.b .* sin(SN.D4.omega(1).*SN.mor.ts) + ...
                SN.D6.a .* cos(SN.D6.omega(1).*SN.mor.ts) + SN.D6.b .* sin(SN.D6.omega(1).*SN.mor.ts) + ...
                SN.D8.a .* cos(SN.D8.omega(1).*SN.mor.ts) + SN.D8.b .* sin(SN.D8.omega(1).*SN.mor.ts);
            
            %%% Scaling of quarter-diurnal amplitudes
            ERR = NaN(length(fac),length(fac));
            for k = 1:length(fac)
                %             fprintf('\t%d/%d\n',k,length(fac));
                for m = 1:length(fac)
                    ...
                    Z2 = ...
                        SN.C1.a .* cos(SN.C1.omega.*SN.mor.ts) + SN.C1.b .* sin(SN.C1.omega.*SN.mor.ts) + ...
                        fD2a .* SN.D2.a .* cos(SN.D2.omega(1).*SN.mor.ts) + fD2b .* SN.D2.b .* sin(SN.D2.omega(1).*SN.mor.ts) + ...
                        fac(k) .* SN.D4.a .* cos(SN.D4.omega(1).*SN.mor.ts) + fac(m) .* SN.D4.b .* sin(SN.D4.omega(1).*SN.mor.ts) + ...
                        SN.D6.a .* cos(SN.D6.omega(1).*SN.mor.ts) + SN.D6.b .* sin(SN.D6.omega(1).*SN.mor.ts) + ...
                        SN.D8.a .* cos(SN.D8.omega(1).*SN.mor.ts) + SN.D8.b .* sin(SN.D8.omega(1).*SN.mor.ts);
            ...
            SN.mor.hC1fD2modfD4D6D8 = ...
                SN.C1.a .* cos(SN.C1.omega.*SN.mor.ts) + SN.C1.b .* sin(SN.C1.omega.*SN.mor.ts) + ...
                fD2a .* SN.D2.a .* cos(SN.D2.omega(1).*SN.mor.ts) + fD2b .* SN.D2.b .* sin(SN.D2.omega(1).*SN.mor.ts) + ...
                fD4a .* SN.D4.a .* cos(SN.D4.omega(1).*SN.mor.ts) + fD4b .* SN.D4.b .* sin(SN.D4.omega(1).*SN.mor.ts) + ...
                SN.D6.a .* cos(SN.D6.omega(1).*SN.mor.ts) + SN.D6.b .* sin(SN.D6.omega(1).*SN.mor.ts) + ...
                SN.D8.a .* cos(SN.D8.omega(1).*SN.mor.ts) + SN.D8.b .* sin(SN.D8.omega(1).*SN.mor.ts);
            
            % Scaling on the diurnal amplitudes
            ERR = NaN(length(fac),length(fac));
            for k = 1:length(fac)
                %             fprintf('\t%d/%d\n',k,length(fac));
                for m = 1:length(fac)
                    clear Z2 dZdt2
                    Z2 = ...
                        fac(k) * SN.C1.a .* cos(SN.C1.omega.*SN.mor.ts) + fac(m) * SN.C1.b .* sin(SN.C1.omega.*SN.mor.ts) + ...
                        fD2a .* SN.D2.a .* cos(SN.D2.omega(1).*SN.mor.ts) + fD2b .* SN.D2.b .* sin(SN.D2.omega(1).*SN.mor.ts) + ...
                        fD4a .* SN.D4.a .* cos(SN.D4.omega(1).*SN.mor.ts) + fD4b .* SN.D4.b .* sin(SN.D4.omega(1).*SN.mor.ts) + ...
                        SN.D6.a .* cos(SN.D6.omega(1).*SN.mor.ts) + SN.D6.b .* sin(SN.D6.omega(1).*SN.mor.ts) + ...
                        SN.D8.a .* cos(SN.D8.omega(1).*SN.mor.ts) + SN.D8.b .* sin(SN.D8.omega(1).*SN.mor.ts);
            ...
            fD1a = fac(idr);
            fD1b = fac(idc);
            SN.mor.hfC1fD2modfD4D6D8 = ...
                fD1a .* SN.C1.a .* cos(SN.C1.omega   .*SN.mor.ts) + fD1b .* SN.C1.b .* sin(SN.C1.omega.*SN.mor.ts) + ...
                fD2a .* SN.D2.a .* cos(SN.D2.omega(1).*SN.mor.ts) + fD2b .* SN.D2.b .* sin(SN.D2.omega(1).*SN.mor.ts) + ...
                fD4a .* SN.D4.a .* cos(SN.D4.omega(1).*SN.mor.ts) + fD4b .* SN.D4.b .* sin(SN.D4.omega(1).*SN.mor.ts) + ...
                SN.D6.a .* cos(SN.D6.omega(1).*SN.mor.ts) + SN.D6.b .* sin(SN.D6.omega(1).*SN.mor.ts) + ...
                SN.D8.a .* cos(SN.D8.omega(1).*SN.mor.ts) + SN.D8.b .* sin(SN.D8.omega(1).*SN.mor.ts);
            ...
            SN.D2.AScaled   = sqrt(fD2a.*SN.D2.a.^2 + fD2b.*SN.D2.b.^2);
            SN.D2.phiScaled = atan((fD2a.*SN.D2.b(1)) / (fD2b.*SN.D2.a(1)));
            
            %%% Scaling of the amplitude of the spring-neap variation
            clear ERR
            ERR = NaN(length(fac),1);
            for k = 1:length(fac)
                clear Z2 dZdt2 Avar
                
                Avar     = fac(k) * SN.D2.A(2) * cos(SN.Dsn.omega.*SN.mor.ts - pi); % -pi to start with neap tide
                Avar     = round(Avar,4);
                Z2 = ...
                    fD1a .* SN.C1.a .* cos(SN.C1.omega   .*SN.mor.ts) + fD1b .* SN.C1.b .* sin(SN.C1.omega.*SN.mor.ts) + ...
                    fac(k) * SN.D2.AScaled .* cos(SN.D2.omega(1).*SN.mor.ts - SN.D2.phiScaled) + ...
                    fD4a .* SN.D4.a .* cos(SN.D4.omega(1).*SN.mor.ts) + fD4b .* SN.D4.b .* sin(SN.D4.omega(1).*SN.mor.ts) + ...
                    SN.D6.a .* cos(SN.D6.omega(1).*SN.mor.ts) + SN.D6.b .* sin(SN.D6.omega(1).*SN.mor.ts) + ...
                    SN.D8.a .* cos(SN.D8.omega(1).*SN.mor.ts) + SN.D8.b .* sin(SN.D8.omega(1).*SN.mor.ts);
            ...
            SN.mor.cycle3 = ...
                fD1a .* SN.C1.a .* cos(SN.C1.omega   .*SN.mor.ts) + fD1b .* SN.C1.b .* sin(SN.C1.omega.*SN.mor.ts) + ...
                fDsn * SN.D2.AScaled .* cos(SN.D2.omega(1).*SN.mor.ts - SN.D2.phiScaled) + ...
                fD4a .* SN.D4.a .* cos(SN.D4.omega(1).*SN.mor.ts) + fD4b .* SN.D4.b .* sin(SN.D4.omega(1).*SN.mor.ts) + ...
                SN.D6.a .* cos(SN.D6.omega(1).*SN.mor.ts) + SN.D6.b .* sin(SN.D6.omega(1).*SN.mor.ts) + ...
                SN.D8.a .* cos(SN.D8.omega(1).*SN.mor.ts) + SN.D8.b .* sin(SN.D8.omega(1).*SN.mor.ts);
        ...
        clear val;
        val         = repmat(SN.mor.cycle3,OPT.nCycles,1);
        ts          = (1:1:length(val))';
    ...
    case 'doubletide'
        ...
        pha2 = rad2deg(mean_angle(deg2rad([D.mor.O1.pha;D.mor.K1.pha])));
